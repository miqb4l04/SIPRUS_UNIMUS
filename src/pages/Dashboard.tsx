...
import { CalendarRange, ClipboardCheck, Clock, FileSpreadsheet, Hourglass, Plus, ShieldCheck, User, Flame, Lightbulb, Bell, Briefcase, GraduationCap, Settings } from 'lucide-react';
...
export default function Dashboard({
  ...
  const fetchStats = async () => {
    try {
      ...
      // Load both bookings lists & buildings to calculate stats
      let allBookings: any[] = [];
      try {
        allBookings = await apiRequest('/booking/all');
      } catch (e) {
        // Fallback if not admin role
        allBookings = await apiRequest('/booking/user');
      }
      ...
      // Calculate bookings per building
  ...
  // Determine current day greeting based on local hours
  ...
  return (
    ...
    >
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="space-y-2">
            <span className="inline-block py-1 px-3 bg-white/10 rounded-full text-xs font-semibold backdrop-blur-md text-yellow-300 tracking-wider">
              {user?.role === 'MAHASISWA' ? 'MAHASISWA WORKSPACE' : user?.role === 'ADMIN_RT' ? 'ADMINISTRASI RT WORKSPACE' : 'SIPRUS KEPALA RT WORKSPACE'}
            </span>
        ...
        <div className="bg-white border border-slate-100 rounded-2xl p-5 shadow-sm flex items-center gap-4 hover:shadow-md transition-shadow">
          ...
          <div>
            <span className="block text-xs font-semibold text-slate-400 uppercase tracking-wider">
              {user?.role === 'MAHASISWA' ? 'Menunggu Validasi' : 'Perlu Validasi Anda'}
            </span>
            {loading ? (
              <span className="block h-7 w-12 bg-slate-100 animate-pulse rounded mt-1"></span>
            ) : (
            ...
            <span className="text-[10px] text-slate-500 block mt-0.5">
              {user?.role === 'MAHASISWA' ? 'Sedang ditinjau petugas' : 'Perlu tindakan verifikasi segera'}
            </span>
          </div>
        </div>
      ...
      {/* DASHBOARD STATISTIK VISUAL & POPULER (Mufti's Analytics Panel) */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6" id="mufti-analytics">
        
        {/* SVG Chart - Sebaran Booking Per Gedung */}
          ...
          {loading ? (
            ...
            <div className="space-y-3.5 pt-2">
              {buildingCounts.map((b, i) => {
                ...
                // Color array mapping for aesthetic variations page
      ...
      <div className="bg-white border border-slate-100 rounded-2xl p-6 shadow-sm space-y-4" id="recent-activity-feed">
        <div className="flex justify-between items-center pb-2 border-b border-slate-100">
          <h3 className="text-sm font-bold text-slate-900 tracking-tight">
            ⚡ Transaksi Reservasi Terbaru (UNIMUS Real-time)
          </h3>
